...
function FloatingParticles({ variant }: { variant: "bureau" | "vieux" }) {
    const [particles, setParticles] = useState<Array<{ id: number, left: number, delay: number, dur: number, size: number, drift: number }>>([]);
    ...
    useEffect(() => {
        ...
        const pts = Array.from({ length: n }, (_, i) => ({
            ...
            delay: Math.random() * 8,
            dur: 3 + Math.random() * 6,
            size: variant === "bureau" ? 2 + Math.random() * 4 : 2.5 + Math.random() * 4.5,
...
function getRingAnimStyle(role: string): React.CSSProperties | undefined {
    if (role === "bureau") return { animation: "pf-ring-gold 3s ease-in-out infinite" };
    if (role === "vieux") return { animation: "pf-ring-purple 3s ease-in-out infinite" };
    return undefined;
}
...
const STATS = [
    { key: "prs_approved", label: "Approved", icon: CheckCircle2, color: "text-emerald-500", bg: "bg-emerald-500/10", bar: "bg-emerald-500" },
    { key: "prs_total", label: "Total PRs", icon: GitPullRequest, color: "text-blue-500", bg: "bg-blue-500/10", bar: "bg-blue-500" },
    { key: "annotations_count", label: "Annotations", icon: Highlighter, color: "text-amber-500", bg: "bg-amber-500/10", bar: "bg-amber-500" },
    { key: "comments_count", label: "Comments", icon: MessageSquare, color: "text-violet-500", bg: "bg-violet-500/10", bar: "bg-violet-500" },
] as const;
...
function StatCard({
    ...
    return (
        ...
        >
            <div className={`absolute inset-x-0 top-0 h-px ${bar} opacity-40 transition-opacity group-hover:opacity-100`} />
    ...
    return (
        ...
        >
            ...
            <div className="space-y-1.5">
                ...
                />
                <p className="text-right text-[10px] text-muted-foreground">{bio.length}/500</p>
            </div>
            <div className="space-y-1.5">
            ...
            <div className="relative z-10 mx-auto w-full max-w-3xl space-y-6 p-4 pb-20 md:p-6 md:pb-6">
                ...
                >
                    ...
                    >
                        ...
                        {special && (
                            <>
                                ...
                                <div className="absolute inset-0 opacity-[0.04] dark:opacity-[0.06] pointer-events-none mix-blend-overlay" style={{ backgroundImage: "url(\"data:image/svg+xml,%3Csvg viewBox='0 0 200 200' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='noiseFilter'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.85' numOctaves='3' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23noiseFilter)'/%3E%3C/svg%3E\")" }} />
                            </>
                        )}
                    ...
                    <div className="relative px-6 pb-6">
                        ...
                        <div className={`group relative ${avatarOffset} mb-4 w-fit ml-1 z-10`}>
                            <Avatar
                        ...
                        <div className="flex flex-wrap items-center gap-2.5 mb-1">
                            <h1
                                className={`flex items-center gap-2 tracking-tight ${
                                    special ? "text-3xl font-black" : "text-2xl font-bold text-foreground"
                                }`}
                ...
                <div style={{ animation: "pf-fade-up 0.5s 0.6s both ease-out" }}>
                    <Tabs value={activeTab} onValueChange={setActiveTab}>
                        <TabsList className="w-full justify-start rounded-xl p-1 h-auto bg-muted/60 dark:bg-muted/30">
                            ...
                            <TabsTrigger value="annotations" className={tabTrigger}>
                                Annotations
                            </TabsTrigger>
                            {showRecentlyViewed && (
                                <TabsTrigger value="recent" className={tabTrigger}>
                                    Recently Viewed
                                </TabsTrigger>
                            )}
                        </TabsList>
                        <TabsContent value="prs" className="mt-4 min-h-[600px]">
                            {activeTab === "prs" && <ContributionList userId={profile.id} type="prs" />}
                        </TabsContent>
                        <TabsContent value="materials" className="mt-4 min-h-[600px]">
                            {activeTab === "materials" && <ContributionList userId={profile.id} type="materials" />}
                        </TabsContent>
                        <TabsContent value="annotations" className="mt-4 min-h-[600px]">
                            {activeTab === "annotations" && <ContributionList userId={profile.id} type="annotations" />}
                        </TabsContent>
                        {showRecentlyViewed && (
                            <TabsContent value="recent" className="mt-4 min-h-[600px]">
