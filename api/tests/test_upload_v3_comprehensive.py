...
@pytest.mark.asyncio
async def test_presigned_multipart_flow(client: AsyncClient, db_session: AsyncSession, fake_redis_setup):
    ...
    with patch("app.routers.upload.presigned.settings.enable_presigned_multipart", True):
        ...
        with (
            patch("app.routers.upload.presigned.create_multipart_upload", new_callable=AsyncMock) as m_create,
            patch("app.routers.upload.presigned.generate_presigned_upload_part", new_callable=AsyncMock) as m_gen_part
        ):
            m_create.return_value = "s3-mp-id"
            m_gen_part.return_value = "http://presigned-part-url"

            response = await client.post("/api/upload/presigned-multipart/init", json=init_data, headers=_auth_headers(user))
        ...
        with (
            ...
            patch("app.routers.upload.presigned._enqueue_processing", new_callable=AsyncMock) as m_enqueue,
        ):
            m_read.return_value = b"%PDF-1.4"
            m_info.return_value = {"size": 20 * 1024 * 1024, "content_type": "application/pdf"}
            response = await client.post("/api/upload/presigned-multipart/complete", json=complete_data, headers=_auth_headers(user))
...
async def test_atomic_cas_ref_counts(fake_redis_setup):
    ...
    from app.workers.process_upload import _LUA_CAS_DECR, _LUA_CAS_INCR
...
async def test_download_file_with_hash_optimization(fake_redis_setup, tmp_path):
    ...
    expected_hash = "796906960cc00099009960bdcc09bc0096cc00099009960bdcc09bc009600099" # Not real, mock it
    expected_hash = "6df600cf860df06900cf860df06900cf860df06900cf860df06900cf860df069" # Still not real
    import hashlib
